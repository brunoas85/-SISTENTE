...
void main() {
  ...
  group('DayCalculator.calculate', () {
    test('jornada exacta: sin deuda ni a favor', () {
      final d = calc.calculate(monday, records: [rec(monday, hm(8, 0), hm(16, 0))]);
    ...
    test('trabajó de más: a favor', () {
      final d = calc.calculate(monday, records: [rec(monday, hm(7, 30), hm(17, 0))]);
    ...
    test('trabajó de menos: deuda no cubierta', () {
      final d = calc.calculate(monday, records: [rec(monday, hm(8, 0), hm(15, 0))]);
    ...
    test('usa la jornada vigente de la fecha', () {
      final c = DayCalculator(schedules: [
        WorkdaySchedule(validFrom: CalendarDate(2026, 6, 1), minutes: 420),
      ]);
      final d = c.calculate(monday, records: [rec(monday, hm(8, 0), hm(15, 0))]);
    ...
    test('usufructo parcial sin fichada cubre solo sus minutos y va a revisión',
        () {
      final d = calc.calculate(
        monday,
        movements: [
    ...
    test('el día de hoy sin fichada ya es faltante', () {
      final c = DayCalculator(today: monday);
      expect(c.calculate(monday).status, DayStatus.missing);
    });
    ...
    test('egreso anterior al ingreso es inválido y no computa', () {
      final d = calc.calculate(monday, records: [rec(monday, hm(16, 0), hm(8, 0))]);
    ...
    test('egreso inválido en uno de varios tramos invalida el día', () {
      final d = calc.calculate(monday, records: [
  ...
  group('inicio del control (primera fichada con la app)', () {
    ...
    test('día hábil anterior al inicio sin fichada no es faltante ni resta', () {
      final d = c.calculate(monday);
    ...
    test('el resumen mensual no resta los días anteriores al inicio', () {
      ...
      // 01/06 antes del inicio, 02/06 jornada exacta, 03/06 faltante.
